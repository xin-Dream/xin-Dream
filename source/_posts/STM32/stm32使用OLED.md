...
在后面有三个文件内容，oled.c  oled.h   oledfont.h，将头文件添加到Inc目录，c文件添加到Src目录

具体使用的时候，需要在main函数开头调用`OLED_Init（）;`，这里用到的IO口不需要在CubeMX中设置，只要调用OLED_Init()，就会对IO口配置。
...
void OLED_Showdecimal(uint8_t x, uint8_t y, float num, uint8_t z_len, uint8_t f_len, uint8_t size2) {
	...
	//小数点
	OLED_ShowChar(x + (size2 / 2) * (z_len), y, '.', size2, 1);

	f_temp = (int) ((num - z_temp) * (mypow(10, f_len)));
	//小数部分
	for (t = 0; t < f_len; t++) {
		temp = (f_temp / mypow(10, f_len - t - 1)) % 10;
		OLED_ShowChar(x + (size2 / 2) * (t + z_len) + 5, y, temp + '0', size2, 1);
	}
}
...
#ifndef INC_002_ARENA_OLEDFONT_H
#define INC_002_ARENA_OLEDFONT_H
...
#endif //INC_002_ARENA_OLEDFONT_H
